...
def gaussian_smoothing(data, feature_name, window_size, std):
...
def evaluation_for_model(df, target):
  print('Evaluating the model for:', target)
  print('-' * 50)

  # Define weight function 
  def weight_function(timestamps):
    weights = np.ones(len(timestamps))  # Equal weights (modify for custom weights)
    return weights
  ...
  smoothed_data = df[features].apply(lambda x: gaussian_smoothing(df.copy(), x.name, 3, 1))
  ...
  weights = weight_function(X_train.index)
...
for each_feature in features:
  evaluation_for_model(df, each_feature)